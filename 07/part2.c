...
int hand_count = 0;
const char *allcards = "J23456789TQKA";
...
int get_type(struct hand h) {
    ...
    for (unsigned int i = 0; i < strlen(allcards); i++) {
        for (unsigned int j = 0; j < strlen(h.cards); j++) {
    ...
    int same1_count = 0, same2_count = 0, joker_count = 0;
    ...
    // handle the jokers
    if (same1_count > same2_count) {
        same1_count += joker_count;
    } else if (same2_count > same1_count) {
        same2_count += joker_count;
    } else if (same1_count == same2_count) {
        // counts are equal, so add the jokers to the first one because the second one might be 0
        same1_count += joker_count;
    }
...
//        printf("%s: %d %s\n", hands[i].cards, hands[i].bid, type2str(hands[i].type));
    ...
    long winnings = 0;
    for (int i = 0; i < hand_count; i++) {
//        printf("%s: %s %d\n", hands[i].cards, type2str(hands[i].type), hands[i].rank);
        assert(hands[i].rank != 0);
