...
long determine_next(int *vals, int count) {
    // keep record is all results are zero
    int is_zero = 1; // true
...
//    printf("\n");
    ...
    long next = 0;
    long last = new_vals[count - 2];
    if (!is_zero) {
        next = last + determine_next(new_vals, count - 1);
        free(new_vals);
    }
//    printf("last is %ld\n", last);
...
long get_next_prediction(int *history, int count) {
//    printf("Element count: %d\n", count);
//    print_history(history, count);

    // now recursively determine the next one for the history
    long last = history[count - 1];
    long next = last + determine_next(history, count); 
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < history_count; i++) {
        long prediction = get_next_prediction(histories[i], value_count);
        sum += prediction;
    }
