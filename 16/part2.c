...
void clear_dirs(void) {
    memset(map_dirs, 0, map_width * map_height * sizeof(char));
}
...
int main(int argc, char *argv[]) {
    ...
    // find the max energized entry point and dir
    int max_energized = 0;
    int max_x = 0, max_y = 0, max_d = 0;
#define set_max(e,x,y,d) {max_energized = (e); max_x = (x); max_y = (y); max_d = (d);}
    // try top and bottom
    for (int x = 0; x < map_width; x++) {
        // from top to bottom
        clear_dirs();
        propagate_light(x, 0, DIR_DOWN);
        int energized = count_energized();
        if (energized > max_energized) set_max(energized, x, 0, DIR_DOWN);
        // from bottom to top
        clear_dirs();
        propagate_light(x, map_height - 1, DIR_UP);
        energized = count_energized();
        if (energized > max_energized) set_max(energized, x, map_height - 1, DIR_UP);
    }
    // try left and right
    for (int y = 0; y < map_height; y++) {
        // from the left to the right
        clear_dirs();
        propagate_light(0, y, DIR_RIGHT);
        int energized = count_energized();
        if (energized > max_energized) set_max(energized, 0, y, DIR_RIGHT);
        // from the right to the left
        clear_dirs();
        propagate_light(map_width - 1, y, DIR_LEFT);
        energized = count_energized();
        if (energized > max_energized) set_max(energized, map_width - 1, y, DIR_LEFT);
    }

    printf("Maximum of energized tiles is [entry = (%d, %d, %c)] %d\n",
           max_x, max_y, dir_chars[max_d], max_energized);
